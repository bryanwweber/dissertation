...
=======
  \def\subsectionautorefname~##1\null{Sub--Section~##1\null}%
>>>>>>> 00c7bfed58c4876a78fd6a467a1e90415e8cc257
  \def\figureautorefname~##1\null{Figure~##1\null}%
  \def\tableautorefname~##1\null{Table~##1\null}%
  \def\equationautorefname~##1\null{Equation~##1\null}%
  \autoref{#1}%
...
Direct determination of the temperature of the gases inside the reaction
chamber during and after compression is difficult. Intrusive techniques
such as thermocouples may introduce hot- or cold-spots in the reaction
chamber, and non-intrusive optical techniques are difficult to set up
and require extensive calibration at the pressures of interest in RCM
studies. Thus, $T_C$ is determined indirectly by applying the assumption
of an adiabatic core region of gases in the reaction chamber
\cite{Mittal2007, Lee1998}. It is assumed that the heat loss from the
reactants only occurs in a thin boundary layer near the wall, and the
central core region is unaffected by heat loss.
...
\label{eq:tfc}
\begin{align}
\ln\left(\text{CR}\right) = \int_{T_0}^{T_{fc}} \! \frac{1}{T\left(\gamma-1\right)} \, \mathrm{d} T \\
\ln\left(\frac{P_{fc}}{P_0}\right) = \int_{T_0}^{T_{fc}} \! \frac{\gamma}{T\left(\gamma-1\right)} \, \mathrm{d} T
\end{align}
\end{subequations}

\noindent
where CR is the volumetric compression ratio, $T_0$ is the initial temperature,
$T_{fc}$ is the temperature at the end of isentropic compression, $\gamma$ is the
temperature-dependent ratio of specific heats, $P_{fc}$ is the pressure at the
end of isentropic compression, and $P_0$ is the initial pressure.

However, this formulation does not account for the finite heat loss from the
core region to the walls and the crevice volume during the compression.
These enthalpy losses will cause the actual pressure and
temperature at the EOC ($P_C$ and $T_C$) to be lower than the wholly
isentropic case. To account for this,
it is assumed that the core region of the gases in the reaction chamber
undergoes isentropic compression \cite{Desgroux1995}. Then \autoref{eq:tfc}
can be applied to the adiabatic core:
...
\noindent
where $P_C$ is the measured pressure at the end of compression.
...
A separate model is required to account for the heat losses to the
reactor walls after the end of compression.

In this work, the CHEMKIN-Pro \cite{Chemkin2012} software is used to
perform the numerical integration and calculation of $T_C$. A volume
trace is computed from the measured pressure trace using the isentropic
relation:
...
\noindent
where $V_1$ and $V_2$ are the volume at consecutive time points,
$P_1$ and $P_2$ are the pressure at consecutive time points, and
$\gamma$ is the temperature dependent specific heat. For use in
\autoref{eq:volume-trace}, $\gamma$ is tabulated for each time
point. Thus, the temperature at each time point must also be
...
in \autoref{fig:piston}.

In general, it is rather difficult to directly measure the temperature
of the reacting gases in the reaction chamber during and after compression.
