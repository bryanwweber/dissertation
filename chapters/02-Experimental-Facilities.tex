...
{Uncertainty of Ignition Delay and Compressed\\ Temperature}
...
linearity is less then 0.2\%, indicating that $B_{P_C}=\SI{0.5}{\bar}$.
...
\Cref{eq:york} is reproduced from \textcite{York2004}, and is presented
...
\begin{table}
\centering
\caption{Symbols in \cref{eq:york}. Reproduced from the work of \textcite{York2004}.}
\label{tab:york-syms}
\begin{tabular}{ll}
\toprule
Symbol & Meaning \\
...
In this section, $X_i$ are the temperatures at which \cref{eq:cp-total}
is evaluated and $Y_i$ are the total specific heats evaluated from \cref{eq:cp-total}.
The weighting of the specific heats---$\omega(Y_i)$---is taken
to be the reciprocal of the uncertainty as calculated by
\cref{eq:cp-uncertainty}. Furthermore, there is no uncertainty in the
...
calculated using the final values of the slope and intercept with
\cref{eq:unc-slope,eq:unc-intercept}).
