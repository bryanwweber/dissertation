...
{U_{C_{p\text{,total}}}}^2 = \left(\frac{\partial C_p}{\partial X_1} U_{X_1}\right)^2 + \ldots + \left(\frac{\partial C_p}{\partial X_n} U_{X_n}\right)^2
\end{equation}
%
...
$C_p$ fits causes approximately $\mathcal{O}\SI{1}{K}$ changes in $T_C$.
...
variation in mole fraction from the expected value; this value is adopted for
the total uncertainty of all liquid fuels. The uncertainty of the gaseous
species' mole fractions can be estimated by the same procedure as in \autoref{sec:unc-p0} since
the same pressure transducer is used to measure the pressure of the species;
the mole fraction is related to the pressure by Dalton's Law of Partial Pressure
