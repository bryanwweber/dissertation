...
in \autoref{fig:rcm-piston}. The boundary layer enters the crevice through
...
corresponding non-reactive pressure trace is used. In \autoref{eq:volume-trace} it is assumed that
...
In \autoref{eq:tc}, the total specific heat ratio for the mixture
...
A schematic of the GSV assembly is shown in \autoref{fig:gsv-assem-schematic}.
...
GSV controller has an adjustable delay \todo{Check this range} (\SIrange{4.5}{50}{\milli\second}) that
...
high speed video. Frames from these videos are shown in \autoref{fig:valve-video}.
...
the sample from the sample loop into the column via the split/splitless injector (\autoref{fig:gcms-valve-inject}).
The valve is then actuated again to remove the sample loop
from the carrier gas path and restore the ``load'' position (\autoref{fig:gcms-valve-load}).
...
their long axes aligned parallel to the ion beam. The ions are admitted
...
and reach the detector to be recorded \cite{Sparkman2011}, generating a mass spectrum. The voltages
are ramped many times per second---typical ramp times range from \SIrange{0.05}{0.5}{\second},
