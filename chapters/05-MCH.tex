...
The pressure profiles for seven experiments with Mix \#2 ($\phi=0.5$) are shown
in \cref{fig:mch-sample}---the orange case is the representative non-reactive
experiment. The abscissa is normalized time, where 1 is the
first stage ignition and 2 is the overall ignition. This requires scaling
the time axis twice. The first scaling is created by normalizing the
time until the first-stage ignition (including the time prior to EOC) by
the first-stage ignition delay. The second scaling is applied to the time
after the first-stage ignition. The time in this period is normalized by
$\tau_2$---the difference between the overall ignition delay and the
first-stage ignition delay---and then added to the first time scaling.

Cases where the diaphragm was punctured prior to the first stage of
ignition were normalized according to the first-stage ignition delay of
the purple case in \cref{fig:mch-sample}. The case where the diaphragm was
punctured after first-stage ignition was normalized by the first-stage
ignition delay of that case for the time prior to the first-stage ignition
delay; the time after the first-stage ignition was normalized by the
difference between the first-stage ignition delay of that case and the
overall ignition delay of the purple case.

The sampling times are chosen to correspond to important points during
the ignition delay:
\begin{itemize}
\item Just prior to EOC
\item Prior to the first-stage ignition
\item During the first-stage ignition
\item Prior to the overall ignition
\item During the overall ignition
\end{itemize}

Representative total ion chromatograms (TICs) corresponding to the
experiments in \cref{fig:mch-sample} are shown in \cref{fig:mch-tic}. 14
peaks are uniquely identified by the GC/MS, including peaks
for important aldehyde and ketone intermediates that are the product of
low-temperature chain branching pathways.

\begin{figure}
\includegraphics[width=11cm]{05-MCH/sample-pressure}
...
\caption{TIC of samples extracted from MCH ignition experiments. The colors
correspond to \cref{fig:mch-sample}. Note that due to an experimental error,
the TIC for \SIlist{26;12}{\milli\second} is cut off prior to the elution
of the MCH peak, but this error was fixed for the \SI{0}{\milli\second} case.
The numbers after each species name represent the value of the matching
parameter returned by the software; peaks with multiple species either had
identical matching parameters or eluted simultaneously.}
\label{fig:mch-tic}
\end{figure}
