...
evident in \autoref{fig:mch-expts} for the $\phi=1.5$ case (Mix \#3) and 
approximately includes the temperature range of $T_C=775–840$ K. For 
$\phi=1.5$, first stage ignition is evident for conditions in the range of 
$T_C=740–800$ K.

For $\phi=1.0$ (Mix \#1), the NTC region of the overall ignition delay could 
...
For the experiments at $\phi=0.5$ (Mix \#2), only three data points in the low 
...
ratio. \Autoref{fig:mch-expts} demonstrates that the $\phi=0.5$ case is the most 
reactive (as judged by the inverse of the ignition delay) and the $\phi=1.5$ 
case is the least reactive. As has been shown for other fuels, including 
\emph{n}-butanol \cite{Weber2011} and Jet-A \cite{Kumar2010}, decreasing the 
...
\begin{figure}
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \caption{Comparison of experimental and simulated ignition delays for three 
            pressures for Mix \#3. The data at $15.1$ and $25.5$ bar are from the 
            study of \textcite{Mittal2009}. (a) First stage ignition delays, 
            (b) overall ignition delays.}
...
At $15.1$ and $25.5$ bar for Mix \#1 and \#2, the overall ignition delay is very 
well predicted for temperatures above approximately 715 K. For lower 
...
predicted to within approximately a factor of two. At $50$ bar, the ignition 
...
values. Furthermore, for all of the equivalence ratios tested at $P_C=50$ bar, 
...
respectively, at $P_C=50$ bar. Also shown in \autoref{fig:mch-pressure} is the 
...
(700 K, 800 K, and 900 K). For the results at 700 K and 800 K, the 
bidirectional sensitivity of the first stage ignition delay to the same 
reactions is also shown, except for two reactions at 800 K for which the 
...
index of the reactions shown is given in parentheses next to the bar. At 700 K, 
the sensitivity of the overall ignition delay is in red and the sensitivity of 
the first stage ignition delay is in blue; at 800 K, the sensitivity of the 
...
stage ignition delay at 800 K is found to be MCH+OH=mchr3+H2O, although it is 
not listed in \autoref{fig:mch-sens}. At 900 K, there is no first stage 
ignition, and thus no sensitivity of the first stage ignition delay. 

Under the pressure/stoichiometry conditions of the present simulations, 800 K 
...
mch2oo=mch2ene+ho2 and (R2) mch2qx+o2=mch2qxqj. For these reactions at 800 K, 
the unidirectional sensitivity index is shown in Fig. 11, where $\tau_{i,2}$ 
