...
The pressure profiles for three experiments with Mix \#2 ($\phi=0.5$) are shown
in \cref{fig:mch-sample} (repeated from \cref{fig:rsa-pressure}). The corresponding
TICs are shown in \cref{fig:mch-tic}. Note that the peak for MCH is not shown
for the black line because of an error in the method that was corrected for later
experiments. 14 peaks are uniquely identified by the GC/MS, including peaks
