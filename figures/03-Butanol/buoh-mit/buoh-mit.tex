% arara: lualatex
\input{../../figure-preamble.tex}
\pgfplotsset{
    every axis/.append style={%
        scale only axis,
        xmin=1.14, xmax=1.35,
        ymin=7, ymax=380,
        table/ignore chars={?},
        table/x=1000/Tc,
    },
    every axis plot/.append style={%
        error bars/y dir=both,
        error bars/y explicit,
    },
}
\pgfplotscreateplotcyclelist{buoh}{%
    only marks,mark=square,black\\%
    only marks,mark=o,black\\%
    only marks,mark=square,blue\\%
    only marks,mark=o,blue\\%
    no markers,dashed,blue\\%
    no markers,dashed,blue\\%
    only marks,mark=square*,gray\\%
    only marks,mark=square*,red\\%
    only marks,mark=*,gray\\%
    only marks,mark=*,red\\%
    no markers,solid,gray\\%
    no markers,solid,red\\%
    no markers,solid,gray\\%
    no markers,solid,red\\%
}
\begin{document}
\begin{tikzpicture}
\pgfplotsset{set layers}
\begin{semilogyaxis}[
    ylabel={Ignition Delay, \si{\milli\second}},
    xlabel={$1000/T_C$, \si{\per\kelvin}},
    ytickten={0,1,2},
    xtick={1.15,1.2,...,1.35},
    log ticks with fixed point,
    minor x tick num=4,
    minor y tick num=8,
    legend style={
        at={(axis cs:1.24,27.5)},
        anchor=north west,
    },
    axis x line*=bottom,
    cycle list name=buoh,
]
\pgfplotstableread{15-bar.txt}\fifbar;
\pgfplotstableread{30-bar.txt}\thibar;
\addplot table[y=Expt, y error=Error] {\fifbar};
\addplot table[y=Expt, y error=Error] {\thibar};
\addplot table[y=Expt, y error=Error] {\fifbar};
\addplot table[y=Expt, y error=Error] {\thibar};
\addplot table[y={create col/linear regression={y=Expt}}] {\fifbar};
\addplot table[y={create col/linear regression={y=Expt}}] {\thibar};
\addplot table[y=Sarathy] {\fifbar};
\addplot table[y=MIT] {15-mit.txt};
\addplot table[y=Sarathy] {\thibar};
\addplot table[y=MIT] {30-mit.txt};
\addplot table[y={create col/linear regression={y=Sarathy}}] {\fifbar};
\addplot table[y={create col/linear regression={y=MIT}}] {15-mit.txt};
\addplot table[y={create col/linear regression={y=Sarathy}}] {\thibar};
\addplot table[y={create col/linear regression={y=MIT}}] {30-mit.txt};
\legend{{\SI{15}{\bar}},{\SI{30}{\bar}},,,Experiments,,,,,,{Sarathy et al. Mechanism},{MIT Mechanism}};
\end{semilogyaxis}
\deflisttick{700,750,...,900}{10}
\edef\temp{%
  \noexpand\pgfplotsset{%
      x tick labels list/.style={xticklabels={\listlabels}}
  }%
}
\temp
\begin{semilogyaxis}[
    title style={yshift=7pt},
    title={\textit{i}-Butanol/O$_2$/N$_2$, $\phi=1.0$},
    axis y line=none,
    axis x line*=top,
    xtick={\listticks},
    x tick labels list,
    minor xtick ={\minorticks}
]
\node[align=left,anchor=west] at (axis cs:1.15,250) {O$_2$ : N$_2$ = 1 : 3.76};
\end{semilogyaxis}
\end{tikzpicture}
\end{document}